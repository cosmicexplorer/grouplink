...
use crate::error::{ErrorCodeable, SignalError, SUCCESS};
use crate::gen::{signal_buffer, signal_crypto_provider, size_t};
...
unsafe fn get_crypto<'a, P: CryptoProvider>(user_data: *mut c_void) -> &'a mut P {
  let user_data: *mut P = mem::transmute::<*mut c_void, *mut P>(user_data);
  assert!(!user_data.is_null());
  &mut *user_data
}

#[no_mangle]
extern "C" fn random_func(data: *mut u8, len: size_t, user_data: *mut c_void) -> c_int {
  let crypto: &mut DefaultCrypto = unsafe { get_crypto(user_data) };
...
unsafe fn get_digest_context<'a, H: Digester<Buffer>>(hmac_context: *mut c_void) -> &'a mut H {
  let hmac_context: *mut H = mem::transmute::<*mut c_void, *mut H>(hmac_context);
  &mut *hmac_context
}
...
extern "C" fn hmac_sha256_update_func(
  ...
  let crypto: &mut DefaultCrypto = unsafe { get_crypto(user_data) };
  let hmac: &mut HMACSHA256 = unsafe { get_digest_context(hmac_context) };
...
#[no_mangle]
extern "C" fn hmac_sha256_cleanup_func(hmac_context: *mut c_void, user_data: *mut c_void) {
  let crypto: &mut DefaultCrypto = unsafe { get_crypto(user_data) };
  let hmac: &mut HMACSHA256 = unsafe { get_digest_context(hmac_context) };
...
extern "C" fn sha512_digest_update_func(
  ...
  let crypto: &mut DefaultCrypto = unsafe { get_crypto(user_data) };
  let sha512: &mut SHA512 = unsafe { get_digest_context(digest_context) };
  let data = unsafe { slice::from_raw_parts(data, data_len as usize) };
...
#[no_mangle]
extern "C" fn sha512_digest_cleanup_func(digest_context: *mut c_void, user_data: *mut c_void) {
  let crypto: &mut DefaultCrypto = unsafe { get_crypto(user_data) };
  let sha512: &mut SHA512 = unsafe { get_digest_context(digest_context) };
...
extern "C" fn encrypt_func(
  ...
  let iv = InitializationVector::from_bytes(unsafe { slice::from_raw_parts(iv, iv_len as usize) });
  let plaintext =
